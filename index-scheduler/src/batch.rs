...
use shared_types::error::Code;
use shared_types::heed::{RoTxn, RwTxn};
use shared_types::search_engine::documents::{obkv_to_object, DocumentsBatchReader};
use shared_types::search_engine::heed::CompactionOption;
use shared_types::search_engine::update::{
    IndexDocumentsConfig, IndexDocumentsMethod, IndexerConfig, Settings as MilliSettings,
};
use shared_types::search_engine::{self, Filter};
use shared_types::settings::{apply_settings_to_builder, Settings, Unchecked};
use shared_types::tasks::{Details, IndexSwap, Kind, KindWithContent, Status, Task};
use shared_types::{compression, Index, VERSION_FILE_NAME};
...
impl IndexScheduler {
    ...
    /// [`finished_at`](shared_types::tasks::Task::finished_at) and [`started_at`](shared_types::tasks::Task::started_at).
    ...
    pub(crate) fn process_batch(&self, batch: Batch) -> Result<Vec<Task>> {
        ...
        match batch {
            ...
            Batch::SnapshotCreation(mut tasks) => {
                ...
                let auth = search_engine::heed::EnvOpenOptions::new()
            ...
            Batch::Dump(mut task) => {
                ...
                for ret in self.all_tasks.iter(&rtxn)? {
                    ...
                    if let Some(content_file) = content_file {
                        ...
                        if status == Status::Enqueued {
                            ...
                            while let Some(doc) =
                                cursor.next_document().map_err(search_engine::Error::from)?
                            {
                ...
                self.index_mapper.try_for_each_index(&rtxn, |uid, index| -> Result<()> {
                    ...
                    for ret in index.all_documents(&rtxn)? {
                        ...
                        let (_id, doc) = ret?;
                        let document = search_engine::obkv_to_json(&all_fields, &fields_ids_map, doc)?;
                        index_dumper.push_document(&document)?;
                    }

                    // 3.2. Dump the settings
                    let settings = shared_types::settings::settings(
                        index,
                        &rtxn,
                        shared_types::settings::SecretPolicy::RevealSecrets,
                    )?;
    ...
    fn apply_index_operation<'i>(
        ...
        match operation {
            IndexOperation::DocumentClear { mut tasks, .. } => {
                let count = search_engine::update::ClearDocuments::new(index_wtxn, index).execute()?;
            ...
            IndexOperation::DocumentOperation {
                ...
                if let Some(primary_key) = primary_key {
                    match index.primary_key(index_wtxn)? {
                        ...
                        Some(pk) => {
                            if primary_key != pk {
                                return Err(search_engine::Error::from(
                                    search_engine::UserError::PrimaryKeyCannotBeChanged(pk.to_string()),
                                )
                                .into());
                            }
                        }
                        ...
                        None => {
                            let mut builder =
                                search_engine::update::Settings::new(index_wtxn, index, indexer_config);
                            builder.set_primary_key(primary_key);
                            builder.execute(
                                |indexing_step| tracing::debug!(update = ?indexing_step),
                ...
                let mut builder = search_engine::update::IndexDocuments::new(
                    index_wtxn,
                    index,
                    indexer_config,
                    config,
                        ...
                        DocumentOperation::Add(content_uuid) => {
                            ...
                            match user_result {
                                ...
                                Err(e) => {
                                    ...
                                    task.error = Some(search_engine::Error::from(e).into());
                                }
                            }
                        }
            ...
            IndexOperation::Settings { index_uid: _, settings, mut tasks } => {
                let indexer_config = self.index_mapper.indexer_config();
                let mut builder = search_engine::update::Settings::new(index_wtxn, index, indexer_config);
...
fn delete_document_by_filter<'a>(
    ...
    Ok(if let Some(filter) = filter {
        let candidates = filter.evaluate(wtxn, index).map_err(|err| match err {
            search_engine::Error::UserError(search_engine::UserError::InvalidFilter(_)) => {
