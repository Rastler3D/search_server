...
use shared_types::search_engine::documents::{DocumentsBatchBuilder, DocumentsBatchReader};
use shared_types::search_engine::update::{IndexDocumentsConfig, IndexDocumentsMethod};
use shared_types::settings::apply_settings_to_builder;
use shared_types::tasks::KindWithContent;
use shared_types::versioning::{check_version_file, create_version_file};
use shared_types::{compression, search_engine, VERSION_FILE_NAME};
pub use option::Opt;
...
pub fn setup_search_server(opt: &Opt) -> anyhow::Result<(Arc<IndexScheduler>, Arc<AuthController>)> {
...
fn import_dump(
    ...
    if let Some(date) = dump_reader.date() {
        tracing::info!(
            version = ?dump_reader.version(), // TODO: get the search_server version instead of the dump version
            %date,
            "Importing a dump of search_server"
        );
    } else {
        tracing::info!(
            version = ?dump_reader.version(), // TODO: get the search_server version instead of the dump version
    ...
    for index_reader in dump_reader.indexes()? {
        ...
        let mut builder = search_engine::update::Settings::new(&mut wtxn, &index, indexer_config);
        ...
        // 4.3.2 We feed it to the search_engine index.
        ...
        let builder = search_engine::update::IndexDocuments::new(
            &mut wtxn,
            &index,
            indexer_config,
            ...
            web::JsonConfig::default()
                ...
                .error_handler(|err, req: &HttpRequest| match err {
                    JsonPayloadError::ContentType => match req.headers().get(CONTENT_TYPE) {
                        Some(content_type) => SearchSystemHttpError::InvalidContentType(
                            content_type.to_str().unwrap_or("unknown").to_string(),
                            vec![mime::APPLICATION_JSON.to_string()],
                        )
                        .into(),
                        None => SearchSystemHttpError::MissingContentType(vec![
