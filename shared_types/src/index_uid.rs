...
impl TryFrom<String> for IndexUid {
    ...
    fn try_from(uid: String) -> Result<Self, Self::Error> {
        if !uid.chars().all(|x| x.is_alphanumeric() || x == '-' || x == '_')
            || uid.is_empty()
            || uid.len() > 400
        {
...
impl fmt::Display for IndexUidFormatError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "Идентификатор индекса `{}` недействителен. Идентификатор документа должен быть либо целым числом, либо строкой с ascii-символами",
            self.invalid_uid,
        )
    }
}
