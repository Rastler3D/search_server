...
use shared_types::error::deserr_codes::InvalidSwapIndexes;
use shared_types::error::ResponseError;
use shared_types::index_uid::IndexUid;
use shared_types::tasks::{IndexSwap, KindWithContent};
use serde_json::json;
...
pub async fn swap_indexes(
    ...
    for SwapIndexesPayload { indexes } in params.into_iter() {
        ...
        let (lhs, rhs) = match indexes.as_slice() {
            [lhs, rhs] => (lhs, rhs),
            _ => {
                return Err(SearchSystemHttpError::SwapIndexPayloadWrongLength(indexes).into());
            }
        };
