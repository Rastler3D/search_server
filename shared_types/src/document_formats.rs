...
use search_engine::documents::{DocumentsBatchBuilder, Error};
use search_engine::Object;
            ...
            Self::MalformedPayload(me, b) => match me {
                Error::Json(se) => {
                    let mut message = match se.classify() {
                        Category::Data => {
                            "данные не являются ни объектом, ни списком объектов".to_string()
                        }
                        _ => se.to_string(),
                    };
                    ...
                    write!(
                        f,
                        "Тело запроса `{}` неправильно сформировано. `Не удалось сериализовать значение документа: {}`.",
                        b, message
                    )
                }
                _ => write!(f, "Тело запроса `{}` неправильно сформировано: `{}`.", b, me),
            },
        }
    }
}
