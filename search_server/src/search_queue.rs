...
impl SearchQueue {
    ...
    pub async fn try_get_search_permit(&self) -> Result<Permit, SearchSystemHttpError> {
        let (sender, receiver) = oneshot::channel();
        self.sender.send(sender).await.map_err(|_| SearchSystemHttpError::SearchLimiterIsDown)?;
        receiver.await.map_err(|_| SearchSystemHttpError::TooManySearchRequests(self.capacity))
    }
    ...
    pub fn health(&self) -> Result<(), SearchSystemHttpError> {
        if self.sender.is_closed() {
            Err(SearchSystemHttpError::SearchLimiterIsDown)
        } else {
            Ok(())
        }
    }
