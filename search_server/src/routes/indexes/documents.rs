...
use shared_types::document_formats::{read_csv, read_json, read_ndjson, PayloadType};
...
use shared_types::search_engine::update::IndexDocumentsMethod;
use shared_types::search_engine::DocumentId;
use shared_types::star_or::OptionStarOrList;
use shared_types::tasks::KindWithContent;
use shared_types::{search_engine, Document, Index};
use mime::Mime;
...
fn extract_mime_type(req: &HttpRequest) -> Result<Option<Mime>, SearchSystemHttpError> {
    match req.mime_type() {
        Ok(Some(mime)) => Ok(Some(mime)),
        Ok(None) => Ok(None),
        Err(_) => match req.headers().get(CONTENT_TYPE) {
            Some(content_type) => Err(SearchSystemHttpError::InvalidContentType(
                content_type.as_bytes().as_bstr().to_string(),
                ACCEPTED_CONTENT_TYPE.clone(),
            )),
            None => Err(SearchSystemHttpError::MissingContentType(ACCEPTED_CONTENT_TYPE.clone())),
        },
    }
}
...
async fn document_addition(
...
) -> Result<SummarizedTaskView, SearchSystemHttpError> {
    let format = match (
        ...
        (Some(("application", "json")), Some(_)) => {
            return Err(SearchSystemHttpError::CsvDelimiterWithWrongContentType(String::from(
                "application/json",
            )))
        }
        ...
        (Some((type_, subtype)), _) => {
            return Err(SearchSystemHttpError::InvalidContentType(
                format!("{}/{}", type_, subtype),
                ACCEPTED_CONTENT_TYPE.clone(),
            ))
        }
        (None, _) => {
            return Err(SearchSystemHttpError::MissingContentType(ACCEPTED_CONTENT_TYPE.clone()))
        }
    };
    ...
    let temp_file = match tempfile() {
        Ok(file) => file,
        Err(e) => return Err(SearchSystemHttpError::Payload(ReceivePayload(Box::new(e)))),
    };
    ...
    while let Some(result) = body.next().await {
        ...
        if byte.is_empty() && buffer_write_size == 0 {
            return Err(SearchSystemHttpError::MissingPayload(format));
        }
    ...
    if let Err(e) = buffer.flush().await {
        return Err(SearchSystemHttpError::Payload(ReceivePayload(Box::new(e))));
    }
...
pub async fn delete_documents_by_filter(
    ...
    || -> Result<_, ResponseError> {
        Ok(crate::search::parse_filter(&filter)?.ok_or(SearchSystemHttpError::EmptyFilter)?)
    }()
...
fn some_documents<'a, 't: 'a>(
    ...
    Ok(index.iter_documents(rtxn, doc_ids)?.map(move |ret| {
        ret.map_err(ResponseError::from).and_then(|(_key, document)| -> Result<_, ResponseError> {
            Ok(search_engine::obkv_to_json(&all_fields, &fields_ids_map, document)?)
        })
    }))
}
...
fn retrieve_document<S: AsRef<str>>(
    ...
    let internal_id = index
        ...
        .get(&txn, doc_id)?
        .ok_or_else(|| SearchSystemHttpError::DocumentNotFound(doc_id.to_string()))?;
    ...
    let document = shared_types::search_engine::obkv_to_json(&all_fields, &fields_ids_map, document)?;
