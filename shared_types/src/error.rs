...
use search_engine::heed::{Error as HeedError, MdbError};
...
impl ResponseError {
    pub fn from_msg(mut message: String, code: Code) -> Self {
        if code == Code::IoError {
            message.push_str(". Эта ошибка обычно возникает, когда на устройстве не остается свободного места или когда база данных не имеет права на чтение или запись.");
        }
        Self {
            code: code.http(),
            message,
...
InvalidAnalyzer                       , InvalidRequest       , BAD_REQUEST ;
NoDefaultAnalyzer                     , InvalidRequest       , BAD_REQUEST ;
...
InvalidSearchShowQueryGraph           , InvalidRequest       , BAD_REQUEST ;
...
InvalidSettingsAnalyzer               , InvalidRequest       , BAD_REQUEST ;
...
InvalidSettingsSplitJoin              , InvalidRequest       , BAD_REQUEST ;
...
FailedToCompileD2                     , InvalidRequest       , BAD_REQUEST ;
...
impl ErrorCode for search_engine::Error {
    fn error_code(&self) -> Code {
        use search_engine::{Error, UserError};
        ...
        match self {
            ...
            Error::UserError(ref error) => {
                match error {
                    ...
                    UserError::InvalidAnalyzer(_) => Code::InvalidAnalyzer,
                    UserError::NoDefaultAnalyzer => Code::NoDefaultAnalyzer
                }
            }
        }
    }
...
impl fmt::Display for ParseOffsetDateTimeError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        writeln!(f, "`{original}` не является допустимой датой. Она должна соответствовать формату RFC 3339. Например, 'YYYY-MM-DD' или 'YYYY-MM-DD HH:MM:SS'.", original = self.0)
    }
}
...
impl std::fmt::Display for InvalidTaskDateError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "`{}` является недопустимым временем. Он должен соответствовать формату даты YYYY-MM-DD или RFC 3339.", self.0)
    }
}
...
impl fmt::Display for DeserrParseBoolError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "не удалось разобрать `{}` как булево, ожидалось либо `true`, либо `false`", self.0)
    }
}
...
impl fmt::Display for DeserrParseIntError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "не удалось разобрать `{}` как целое положительное число", self.0)
    }
}
...
impl fmt::Display for deserr_codes::InvalidSearchSemanticRatio {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "значение `semanticRatio` недопустимо, ожидается значение между `0.0` и `1.0`."
        )
    }
}
