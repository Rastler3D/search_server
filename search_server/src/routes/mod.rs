...
use shared_types::settings::{Settings, Unchecked};
use shared_types::tasks::{Kind, Status, Task, TaskId};
...
pub fn configure(cfg: &mut web::ServiceConfig) {
    cfg.service(web::scope("/tasks").configure(tasks::configure))
        ...
        .service(web::scope("/graph-render").configure(graph_render::configure))
        .service(web::scope("/analyzer-schema").configure(analyzer_schema::configure));
...
pub async fn running() -> HttpResponse {
    HttpResponse::Ok().json(serde_json::json!({ "status": "SearchSystem is running" }))
}
